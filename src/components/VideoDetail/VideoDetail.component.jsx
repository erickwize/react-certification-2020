...
function VideoDetail({ currVid, setCurrVid }) {
  /* const response = {
  ...
  console.log('VideoDetail:state', state);
  return (
    <Wrapper>
      {hasItems(currVid) && (
        <>
          <VideoPlayer currVid={currVid} />
          <div>
            <h3>Related videos</h3>
            {state && (
              <VideoList items={state.items} currVid={currVid} setCurrVid={setCurrVid} />
            )}
          </div>
        </>
      )}
